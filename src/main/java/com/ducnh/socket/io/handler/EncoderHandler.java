...
import com.ducnh.socket.io.messages.HttpMessage;
import com.ducnh.socket.io.messages.OutPacketMessage;
import com.ducnh.socket.io.messages.XHROptionsMessage;
import com.ducnh.socket.io.messages.XHRPostMessage;
...
public class EncoderHandler extends ChannelOutboundHandlerAdapter{
		...
		while (resources.hasMoreElements()) {
			try (InputStream inputStream  = resources.nextElement().openStream()) {
				...
				if (name != null && name.equals("netty-socketio")) {
					version = name + "/" + attrs.getValue("Bundler-Vesion");
					break;
				}
			} catch (IOException e) {
	...
	private void sendMessage(HttpMessage msg, Channel channel, ByteBuf out, String type, ChannelPromise promise, HttpResponseStatus status) {
		...
		if (userAgent != null && (userAgent.contains(";MISE") || userAgent.contains("Trident/"))) {
